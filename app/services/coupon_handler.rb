...
class CouponHandler < BaseService
  ...
  def apply
    ...
    return return_with(:error, "This coupon has already been used.") unless valid_coupon?
  ...
  def valid_order?
    (original_price >= coupon.minimum_order) && (order.coupon.nil?)
  end
  ...
  def coupon_discount
    if coupon.unit == :value
      ...
      original_price - (original_price * coupon.discount / 100)
    else
      0
    end
  end
