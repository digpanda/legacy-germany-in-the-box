  ...
  def checkout
    ...
    if status
      ...
      @wirecard = PrepareOrderForWirecardCheckout.perform({
        ...
        :user        => current_user,
        :order       => order,
        # IN CASE THE merchant_id don't work (i don't know cart.submerchant_id) YOU MUST KNOW THIS SHOULD MATCH THE shop.merchant_id which's a unique id to recognize merchants through wirecard. - Laurent, 2016/07/05
        :merchant_id => Rails.env.production? ? cart.submerchant_id : 'dfc3a296-3faf-4a1d-a075-f72f1b67dd2a',
