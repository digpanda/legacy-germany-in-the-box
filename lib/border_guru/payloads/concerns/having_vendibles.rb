  ...
  module Payloads
    module HavingVendibles
      ...
      def product_summaries(order)
        {
          subtotal: order.total_price_with_discount.to_f.round(2),
      ...
      def adjusted_price(order)
        order.order_items.inject(0) do |acc, current_order_item|
          acc = acc + current_order_item.price_with_coupon_applied.to_f.round(2)
        end
      end
      ...
      def subtotal_difference(order)
        order.total_price_with_discount.to_f.round(2) - adjusted_price(order)
      end
      ...
      def adjusted_order_item_price(order_item, index)
        if index == 0
          order_item.price_with_coupon_applied.to_f.round(2) + subtotal_difference(order_item.order)
        else
          order_item.price_with_coupon_applied.to_f.round(2)
        end
      end
