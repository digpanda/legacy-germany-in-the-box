...
describe CouponHandler, :type => :request do
  ...
  it "should apply and remove the coupon from an order" do
    CouponHandler.new(IdentitySolver.new(request, @order.user), @coupon, @order).apply
    expect(@order.coupon).to eq(@coupon)
    CouponHandler.new(IdentitySolver.new(request, @order.user), @coupon, @order).unapply
  ...
  it "should apply multiple times if it is not unique" do
    CouponHandler.new(IdentitySolver.new(request, @order.user), @reusable_coupon, @order).apply
