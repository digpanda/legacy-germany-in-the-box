...
describe CouponHandler, :type => :request do
  ...
  it "should apply and remove the coupon from an order" do
    CouponHandler.new(IdentitySolver.new(request, @order.user, session), @coupon, @order).apply
    expect(@order.coupon).to eq(@coupon)
    CouponHandler.new(IdentitySolver.new(request, @order.user, session), @coupon, @order).unapply
  ...
  it "shouldn't apply to more than one order when is unique" do
    CouponHandler.new(IdentitySolver.new(request, @order.user, session), @coupon, @order).apply
    expect(@order.coupon).to eq(@coupon)
    CouponHandler.new(IdentitySolver.new(request, @order_2.user), @coupon, @order_2).apply
    expect(@order_2.coupon).to be(nil)
  end

  it "should apply multiple times if it is not unique" do
    CouponHandler.new(IdentitySolver.new(request, @order.user, session), @reusable_coupon, @order).apply
    expect(@order.coupon).to eq(@reusable_coupon)
    @coupon.update(last_used_at: Time.now)
    CouponHandler.new(IdentitySolver.new(request, @order_2.user), @reusable_coupon, @order_2).apply
