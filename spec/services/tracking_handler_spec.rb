describe TrackingHandler do
  ...
  context '#refresh!' do
    ...
    before(:each) do
      ...
      allow_any_instance_of(described_class).to receive(:api_performed).and_return(kuaidi_valid_response)
    end
    ...
    it 'returns a wrong API result' do
      ...
      allow_any_instance_of(described_class).to receive(:api_performed).and_return(kuaidi_wrong_response)
      refreshed = subject.refresh!
