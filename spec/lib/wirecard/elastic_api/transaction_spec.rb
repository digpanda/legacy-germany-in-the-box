describe Wirecard::ElasticApi::Transaction do
  ...
  context "#response" do
    ...
    it 'should return a response hash' do
      ...
      VCR.use_cassette("wirecard-api-transaction", :record => :new_episodes) do
        response = Wirecard::ElasticApi::Transaction.new(TEST_MERCHANT, TEST_TRANSACTION, TEST_PAYMENT_METHOD).response.raw
    ...
    it 'should return a formatted response' do
      ...
      VCR.use_cassette("wirecard-api-transaction", :record => :new_episodes) do
        response = Wirecard::ElasticApi::Transaction.new(TEST_MERCHANT, TEST_TRANSACTION, TEST_PAYMENT_METHOD).response
    ...
    it 'should not find the transaction and raise an error' do
      ...
      VCR.use_cassette("wirecard-api-transaction", :record => :new_episodes) do
        expect{Wirecard::ElasticApi::Transaction.new(TEST_MERCHANT, "fake-transaction", TEST_PAYMENT_METHOD).response.raw}.to raise_error(Wirecard::ElasticApi::Error)
      end
  ...
  context "#status" do
    ...
    it 'should return the status' do
      ...
      VCR.use_cassette("wirecard-api-transaction", :record => :new_episodes) do
        transaction_state = Wirecard::ElasticApi::Transaction.new(TEST_MERCHANT, TEST_TRANSACTION, TEST_PAYMENT_METHOD).response.transaction_state
