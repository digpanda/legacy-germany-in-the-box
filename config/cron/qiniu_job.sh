...
Stepcount=1
PS3='Preparation: Please enter your step $Stepcount of 8'
options=("export prod bucket file list" "process prod bucket file list" "export prod-backup bucket file list" "process prod-backup bucket file list" "export staging bucket file list" "process staging bucket file list" "export staging-backup bucket file list" "process staging-backup bucket file list" "Quit")
select opt in "${options[@]}"
do
    case $opt in
        "export prod bucket file list")
            echo "you chose choice $Stepcount"
            ((Stepcount++))
        ...
        "process staging-backup bucket file list")
            ...
            echo "file list export and processing is done!"
            break
            ;;            
        "Quit")
            echo "Oops, bye bye!"
...
PS3='Please enter your in steps from 1 to 6:'
options=("clean prod backup" "backup prod" "clean staging" "prod to staging" "Quit")
select opt in "${options[@]}"
do
    case $opt in
        "export prod bucket file list")
